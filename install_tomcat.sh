...
do
	if [ -d $path"/"$folder"/WebRoot" ]; then
		if [ -f $path"/"$folder"/WebRoot/service.conf" ]; then
			...
			do
				cp $json $path/config/
				sed -i "s/8080/$((8800+$c))/g" $path/config/$json
				sed -i "s#123456#xiaole001#g" ./WEB-INF/classes/jdbcDataSource.xml 2> /dev/null
				sed -i "s#libuser.service.dc1.consul#node1#g" ./WEB-INF/classes/jdbcDataSource.xml 2> /dev/null
				sed -i "s#libexam.service.dc1.consul#node1#g" ./WEB-INF/classes/jdbcDataSource.xml 2> /dev/null
				sed -i "s#libmessage.service.dc1.consul#node1#g" ./WEB-INF/classes/jdbcDataSource.xml 2> /dev/null

				sed -i "s#123456#xiaole001#g" ./WEB-INF/classes/jdbc.properties 2> /dev/null
