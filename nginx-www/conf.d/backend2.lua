...
if ngx.var.cookie_NOONLYSESSION ~= nil then
	...
	for _,v in pairs(body.message) do 
		if cache:sismember("likes:"..userid,v.id) == 1 then
			v.islike = "true"
		end
		
		--local like = cache:keys("like_*_*_*_"..v.id);
		--if like ~= nil then
		--	v.likes = table.getn(like)
		--else
		--	v.like = "0"
		--end
		--local reply = cache:keys("reply_*_*_*_"..v.id);
                --if reply ~= nil then
                --        v.reply = table.getn(reply)
                --else
                --        v.reply = "0"
                --end
	end
